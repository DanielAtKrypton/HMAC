...
if (~isempty(key)) && (length(key) < Blocksize)
...
i_pad_bin = cell(1, Blocksize);
o_pad_bin = cell(1, Blocksize);
for i = 1:Blocksize
    i_pad_bin{1,i} = i_pad;
    o_pad_bin{1,i} = o_pad;
end
...
for i = 1:length(str)    
    temp = dec2bin(str2double(sprintf('%u',str(i))),8);
    for j = 1:8
        bin{1,i}(1,j) = str2double(temp(j));
    end
end
...
if length(a) == length(b)
    ...
    ME = MException('HMAC:bit8xor', ...
        sprintf('%s','Input cells must be the same length'));
    throw(ME)
end
